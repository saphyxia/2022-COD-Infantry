...
void INA_Read_Byte_s_0(unsigned char reg,unsigned char *data)	//读两位数据
{
	IIC_Start();
...
void INA_Read_Byte_s_1(unsigned char reg,unsigned char *data)	//读两位数据
{
	IIC_Start();
	IIC_Send_Byte(INA226_I2C_ADDRESS_CONF_1);//发送INA219地址s
	IIC_Wait_Ack();
	IIC_Send_Byte(reg);
	IIC_Wait_Ack();
	IIC_Start();
	IIC_Send_Byte(INA226_I2C_ADDRESS_CONF_1+0x01);	//设置iic为接收模式，将bit0置1将变换此时信号
	IIC_Wait_Ack();
	*data=IIC_Read_Byte(1);
	data++;
	*data=IIC_Read_Byte(0);
...
void INA_REG_Write_0(unsigned char reg,unsigned int data)	//写寄存器		测试成功
{
...
void INA_REG_Write_1(unsigned char reg,unsigned int data)	//写寄存器		测试成功
{
	unsigned char data_temp[2];
	data_temp[0]=(unsigned char )(data>>8);
	data_temp[1]=(unsigned char )(data & 0xFF);
	IIC_Start();
	IIC_Send_Byte(INA226_I2C_ADDRESS_CONF_1);	//发送INA219地址
	IIC_Wait_Ack();
	IIC_Send_Byte(reg);							//发送寄存器地址
	IIC_Wait_Ack();
	IIC_Send_Byte(data_temp[0]);						//发送高8位数据
...
uint16_t INA_GET_Voltage_MV_0(void)	//获取电压（单位：mv）
{
	unsigned char data_temp[2];
	INA_REG_Write_0(INA226_Config_Reg_ADDR,INA226_CONFIG_value);
	INA_REG_Write_0(INA226_Calibration_Reg_ADDR,INA_CAL);
...
int16_t INA_GET_Current_MA_0(void)		//获取电流（单位：mA）
{
	unsigned char data_temp[2];
	INA_REG_Write_0(INA226_Config_Reg_ADDR,INA226_CONFIG_value);
	INA_REG_Write_0(INA226_Calibration_Reg_ADDR,INA_CAL);
...
int16_t INA_GET_Power_MW_0(void)		//获取当前功率（单位：mw）
{
	unsigned char data_temp[2];
	INA_REG_Write_0(INA226_Config_Reg_ADDR,INA226_CONFIG_value);
	INA_REG_Write_0(INA226_Calibration_Reg_ADDR,INA_CAL);
...
int16_t INA_GET_Voltage_MV_1(void)	//获取电压（单位：mv）
{
	unsigned char data_temp[2];
	INA_REG_Write_1(INA226_Config_Reg_ADDR,INA226_CONFIG_value);
	INA_REG_Write_1(INA226_Calibration_Reg_ADDR,INA_CAL);
...
int16_t INA_GET_Current_MA_1(void)		//获取电流（单位：mA）
{
	unsigned char data_temp[2];
	INA_REG_Write_1(INA226_Config_Reg_ADDR,INA226_CONFIG_value);
	INA_REG_Write_1(INA226_Calibration_Reg_ADDR,INA_CAL);
...
int16_t INA_GET_Power_MW_1(void)		//获取当前功率（单位：mw）
{
	unsigned char data_temp[2];
	INA_REG_Write_1(INA226_Config_Reg_ADDR,INA226_CONFIG_value);
	INA_REG_Write_1(INA226_Calibration_Reg_ADDR,INA_CAL);
