...
//INA配置的参数
#define INA_R 0.1f									//阻值大小（单位ohm）
#define INA_I_MAX	3								//硬件最大检测电流（单位：A）	INA219_CONFIG_GAIN_电压/INA_R
#define IAN_I_LSB	0.1f							//(0x04)检测出值对应每位电流值	选择接近 = (INA_I_MAX/32767(2^15))*1000(为了得到单位为A)		(A)
#define INA_V_LSB 4
...
typedef struct
{
    int16_t(*voltage_mv)(void);
    int16_t(*current_ma)(void);
    int16_t(*power_mw)(void);
}INA219_GET_DATA_T;

extern INA219_GET_DATA_T INA219_GET;
