# 1 "..\\user\\_Bsp\\usart2.c"
# 1 "..\\user\\_Bsp\\usart2.h"
# 1 "..\\system\\sys.h"
...
void JY901_Data_Realize(unsigned char ucData);





# 2 "..\\user\\_Bsp\\usart2.c"
...
int yaw_sign,yaw_int,yaw_decimal,pitch_sign,pitch_int,pitch_decimal;
float a[3],w_xyz[3],T;
...
void usart2_Init(u32 bound)
{
    ...
    GPIO_InitStructure.GPIO_Pin = ((uint16_t)0x0004) | ((uint16_t)0x0008); 
...
void USART2_IRQHandler(void)
{
    ...
    {   
        USART_SendData(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x1400)), TxBuffer[TxCounter++]); 
        USART_ClearITPendingBit(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x1400)), ((uint16_t)0x0727));
        if(TxCounter == count) USART_ITConfig(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x1400)), ((uint16_t)0x0727), DISABLE);
    }
    else if(USART_GetITStatus(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x1400)), ((uint16_t)0x0525)) != RESET)
    {
        JY901_Data_Realize((unsigned char)((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x1400))->DR);
        USART_ClearITPendingBit(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x1400)), ((uint16_t)0x0525));
    }

    USART_ClearITPendingBit(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x1400)),((uint16_t)0x0360));
...
void JY901_Data_Realize(unsigned char ucData)
{
    ...
    if (ucRxCnt<11) {return;}
        ...
        switch(ucRxBuffer[1])
        {
            ...
            pitch_Angle = (float)stcAngle.Angle[0]/32768*180;
            roll_Angle = (float)stcAngle.Angle[1]/32768*180;
            yaw_Angle = (float)stcAngle.Angle[2]/32768*180;

            if(yaw_Angle>=0) yaw_sign=1;
            if(yaw_Angle<0)  yaw_sign=0; 
            yaw_int=( (((int)yaw_Angle)>0) ? (((int)yaw_Angle)):(-((int)yaw_Angle)) );
            yaw_decimal=( (((int)(yaw_Angle*100)%100)>0) ? (((int)(yaw_Angle*100)%100)):(-((int)(yaw_Angle*100)%100)) );

            if(pitch_Angle>=0) pitch_sign=1;
            if(pitch_Angle<0)  pitch_sign=0; 
            pitch_int=0;
            pitch_decimal=0;


            roll_Gyro=(double)stcGyro.w[0]/32768*1000;	
            pitch_Gyro=(double)stcGyro.w[1]/32768*1000;	
            yaw_Gyro=(double)stcGyro.w[2]/32768*1000;	
            break;
        }
        ucRxCnt=0;
    }
