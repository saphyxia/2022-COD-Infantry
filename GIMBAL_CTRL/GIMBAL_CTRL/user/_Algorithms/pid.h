...
|~~    ~~|--~~~~--_ \     ~==-/   | \~--===~~        .\                                   传入PID_Init_ByParamArray函数的参数2.另一种就是直接形参赋值
            '         ~-|      /| C  |-~\~~       __--~~                            2022-1-11改动日志----武文斌
                        |-~~-_/ | O  |   ~\_   _-~            /\                           将全部函数改成可以直接外部调用。但依旧保留函数指针
                            /  \  D  \__   \/~                \__
                        _--~ _/ | .-~~____--~-/                  ~~==.
                        ((->/~   '.|||' -_|    ~~-/ ,              . _||
                                -_     ~\      ~~---l__i__i__i--~~_/
...
void PID_Parameter_Init(PID_PARAMETER*,float p,float i ,float d,float e_M,float I_M,float P_OM,float I_OM,float D_OM,float T_OM);
float PID_Calculating(PID_MIDDLE*p,float err);
