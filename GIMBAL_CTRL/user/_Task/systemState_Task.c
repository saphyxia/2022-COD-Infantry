...
void SYSTEMSTATE_TASK(void *args)
{
	...
	while(1)
	{
		myprintf(ABS(shoot.wheel_L->Data.velocity-shoot.wheel_R->Data.velocity)/2.f*100,robot.bullet_speed*100);
...
void Referee_get_Data(CAN_TypeDef* CANx,CanRxMsg* RxMsg)
{
	if(RxMsg->StdId!=0x302)
	{
		...
		uint8_t mode_L = (RxMsg->Data[0] & (uint8_t)(1 << 2)) >> 2,
				mode_H = (RxMsg->Data[0] & (uint8_t)(1 << 3)) >> 3; //发射机构类型：00(初始)，01(爆发)，10(冷却)，11(弹速)
		uint8_t robot_LV_L = (RxMsg->Data[0] & (uint8_t)(1 << 4)) >> 4,
				robot_LV_H = (RxMsg->Data[0] & (uint8_t)(1 << 5)) >> 5;//等级情况，00(1级) 01(2级) 10(3级)
	
		robot.id = (RxMsg->Data[0] & (uint8_t)(3 << 6)) >> 6;//0蓝2红
		robot.mains_power_shooter_output = (RxMsg->Data[0] & (uint8_t)(1 << 1)) >> 1; //主控电源输出情况：1为24v，0为无输出
