...
{
    p->X_mid =p->A*p->X_last;                     //上一次的估计值=系数*上一次的估计值
    p->P_mid = p->A*p->P_last+p->Q;               //上一次估计误差=系数*上一次的估计误差+估计值方差
    p->kg = p->P_mid/(p->P_mid+p->R);             //卡尔曼增益=上一次的估计误差/(上一次的估计误差+测量值方差)
    p->X_now = p->X_mid+p->kg*(dat-p->X_mid);     //当前估计值=上一次的估计值+卡尔曼增益*(当前测量值-上一次的估计值)
    p->P_now = (1-p->kg)*p->P_mid;                //当前估计误差=(1-卡尔曼增益)*上一次的估计误差
    p->P_last = p->P_now;                         //上一次估计误差更新
    p->X_last = p->X_now;                         //上一次估计值更新
    return p->X_now;							                //输出估计结果
}
